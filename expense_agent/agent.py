        ...
        try:
            async with stdio_client(server_params) as (read, write):
                async with ClientSession(read, write) as session:
                    ...
                    result = await session.call_tool("get_expense_policy", arguments={"category": category})
...
def parse_expense(node_input: Any) -> Event:
    ...
    return Event(output=expense.model_dump_json(), route="categorize", state=state_updates)
...
def auto_approve(ctx: Context, node_input: ReviewOutput) -> Event:
    ...
    result = {"status": "approved", "reason": "auto-approved: no risks found by review agent", "details": node_input.model_dump()}
...
root_agent = Workflow(
    ...
    edges=[
        ...
        Edge(from_node=security_screen, to_node=human_review, route="injection_detected"),
